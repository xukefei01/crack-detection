...
def load_ECAG_vgg16(model_path):
    model = ECAG_vgg16()
    checkpoint = torch.load(model_path)
    if 'model' in checkpoint:
        model.load_state_dict(checkpoint['model'])
...
def load_ECA_mobile(model_path):
    model = ECA_mobile()
    checkpoint = torch.load(model_path)
    if 'model' in checkpoint:
        model.load_state_dict(checkpoint['model'])
...
def load_unet_ecaresnet_34(model_path):
    model = ECAUNetResNet(pretrained=False, encoder_depth=34, num_classes=1)
    checkpoint = torch.load(model_path)
    if 'model' in checkpoint:
        model.load_state_dict(checkpoint['model'])
    elif 'state_dict' in checkpoint:
