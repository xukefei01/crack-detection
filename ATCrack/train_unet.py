...
from unet.AttU_net import AttU_Net
from unet.ECA_resnet_unet import ECAUNetResNet
from evaluation_index import *
from unet.ECAG_net import ECA_AG_net
from unet.ECAG_vgg16 import ECAG_vgg16
from unet.ECA_net import ECA_net
from unet.ECAG_mobilenet import ECA_mobile
from unet.AT_mobile import AT_mobilev2
...
def create_model(device, type ='vgg16'):    # 建立模型
    if type == 'vgg16':
        ...
        print('create AttU_net model')
        model = AttU_Net()
    elif type == 'ecaresnet101':
        encoder_depth = 101
        num_classes = 1
        print('create ecaresnet101 model')
        model = ECAUNetResNet(encoder_depth=encoder_depth, num_classes=num_classes, pretrained=False)
    elif type == 'ecaresnet34':
        encoder_depth = 34
        num_classes = 1
        print('create ecaresnet34 model')
        model = ECAUNetResNet(encoder_depth=encoder_depth, num_classes=num_classes, pretrained=False)
    elif type == 'ecagnet':
        print('create ECAG_NET model')
        model = ECA_AG_net(pretrained=False)
    elif type == 'ECAG_vgg16':
        print('create ECAG_vgg16 model')
        model = ECAG_vgg16(pretrained=False)
    elif type == 'ECA_net':
        print('create ECA_net model')
        model = ECA_net(pretrained=False)
    elif type == 'ECA_mobile':
        print('create ECA_mobile model')
        model = ECA_mobile(pretrained=False)
    elif type == 'AT_mobile':
        print('create AT_mobilev2 model')
        model = AT_mobilev2(pretrained=False)
    else:
        assert False
    model.eval()
...
def adjust_learning_rate(optimizer, epoch, lr):  # 调整学习速率
...
def find_latest_model_path(dir):   # 恢复训练时进入最新一代模型训练
    model_paths = []
    epochs = []
...
def train(train_loader, model, criterion, optimizer, validation, args):
    ...
    for epoch in range(epoch, args.n_epoch + 1):
        ...
        tq = tqdm.tqdm(total=(len(train_loader) * args.batch_size))         # 增加进度条
...
if __name__ == '__main__':
    ...
    parser.add_argument('-batch_size',  default=4, type=int,  help='train image number (default: 4)')
    ...
    parser.add_argument('-model_type', type=str, required=False, default='resnet101',
                        choices=['vgg16', 'resnet101', 'resnet34', 'AttU_net',
                                 'ecaresnet101', 'ecaresnet34', 'ecagnet', 'ECAG_vgg16',
                                 'ECA_net', 'ECA_mobile', 'AT_mobile'])
