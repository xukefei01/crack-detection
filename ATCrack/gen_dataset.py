...
def creat_dataset(image_num=1000):
    image_list = os.listdir(r'E:\dataset\CFD\images/')
    masks_list = os.listdir(r'E:\dataset\CFD\masks/')
    print('creating dataset...')
    image_each = image_num / len(image_list)
    label_each = image_num / len(masks_list)
    g_count = 0
    for i in tqdm(range(len(image_list))):
        count = 0
        # image ->3 channels
        src_img = cv2.imread('E:\dataset\CFD\images/' + image_list[i])
        # label ->single channel
        label_img = cv2.imread('E:\dataset\CFD\masks/' + image_list[i], cv2.IMREAD_GRAYSCALE)
        ...
        while count < image_each:
            ...
            cv2.imwrite(('E:\dataset\CFDDD\images/%d.jpg' % g_count), src_aug)
            cv2.imwrite(('E:\dataset\CFDDD\masks/%d.jpg' % g_count), label_aug)
            count += 1 
            g_count += 1
